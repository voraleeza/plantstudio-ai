...
function normalizeOutput(out,size){
  const raw=out.data,dims=out.dims||[],count=size*size,off=Math.max(0,raw.length-count),m=new Float32Array(count);
...
export default async function handler(req,res){
  if(req.method==='GET')return res.status(200).json({ok:true,engine:'Foreground composition: IS-Net + MediaPipe human preservation',ready:true});
  ...
  }catch(e){console.error('Foreground composition remover failed:',e);return res.status(500).json({error:e?.message||'Background removal failed'})}
}
