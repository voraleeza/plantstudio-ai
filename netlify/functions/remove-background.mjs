export default async (request) => {
  ...
  // Health check used by PlantStudio so a failed function deployment is obvious.
  ...
  if (!apiKey) {
    return new Response(
      JSON.stringify({
        error: "PHOTOROOM_API_KEY is not configured in Netlify environment variables.",
      }),
  ...
  try {
    ...
    // Keep below PhotoRoom's documented 50 MB input limit.
    if (imageBytes.byteLength > 50 * 1024 * 1024) {
      return new Response(
        JSON.stringify({ error: "Image is larger than PhotoRoom's 50 MB limit." }),
    ...
    formData.append(
      "image_file",
      new Blob([imageBytes], { type: inputType }),
      fileName
    );

    // Transparent PNG, preserve PhotoRoom's own matting, don't crop.
    ...
    if (!photoRoomResponse.ok) {
      ...
      if (contentType.includes("application/json")) {
        ...
        try {
          detail = (await photoRoomResponse.text()).slice(0, 500);
        } catch {}
      }

      const friendly =
        photoRoomResponse.status === 402
    ...
    return new Response(
      JSON.stringify({
        error: "Background removal failed on the server. Please try again.",
      }),
      { status: 500, headers: { "content-type": "application/json" } }
    );
  }
